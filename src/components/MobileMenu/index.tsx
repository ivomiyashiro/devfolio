...
const MobileMenu = ({ isOpen, handleMenuOpen }: Props) => {
  console.log(isOpen)
  return (
    <menu className={
      `block w-full tablet:hidden h-screen fixed top-0 left-0 px-4 transition-all ${isOpen ? 'visible z-20' : 'translate-x-[-100%] delay-[.35s]'}
       ...
       before:content-[""] before:h-full before:left-0 before:absolute before:transition-transform before:duration-[.35s] before:w-full before:z-20 before:bg-background-100 ${isOpen ? 'before:translate-x-0 before:delay-[.15s]' : 'before:translate-x-[-100%] before:delay-[.15s]'}
      `
    }>
      <ul className={`opacity-0 p-0 transition-all duration-[.3s] absolute top-[50%] z-50 translate-y-[-50%] left-[15%] ${isOpen ? 'opacity-100 delay-[.4s] translate-y-[-60%]' : ''}`}>
        { NAVBAR_DATA.map(({ text, to }, index) => (
          <li key={ index } className="align-items-center cursor-pointer flex text-base font-semibold gap-6 py-2 px-1.5 relative transition duration-200">
            <a href={to} onClick={() => { handleMenuOpen(false) }}>
              <div className="flex items-center gap-[0.5em] px-[12px] py-[20px]">
                <p className="font-mono text-[1.65rem] font-semibold hover:text-textColor-300 transition-colors">{ text }</p>
