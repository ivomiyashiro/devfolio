...
const Header = ({ i18n }: { i18n: string[] }) => {
  ...
  return (
    <header
      id="header-nav"
      className={`flex fixed top-0 items-center bg-transparent h-20 justify-between w-full z-50 transform transition-transform duration-250 tablet:h-[70px] ${visible ? 'tablet:translate-y-0' : 'tablet:-translate-y-full'}`}
    >
